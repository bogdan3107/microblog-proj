...
def translate_text():
    ...
    response_data = {'text': translation}
...
@bp.route('/send_message/<recepient>', methods=['GET', 'POST'])
@login_required
def send_message(recepient):
    user = User.query.filter_by(username=recepient).first_or_404()
    form = MessageForm()
    if form.validate_on_submit():
        msg = Message(author=current_user, recepient = user,
                      body = form.message.data)
        db.session.add(msg)
        user.add_notification('unread_message_count', user.new_messages())
        db.session.commit()
        flash(_('Your message has been sent'))
        return redirect(url_for('main.user', username=recepient))
    return render_template('send_message.html', title=_('Send message'),
                           form=form, recepient=recepient)
...
def export_posts():
    if current_user.get_task_in_progress('export_posts'):
        flash(_('An export task is currently is progress'))
    else:
